...
test('identifies as the codex harness', function () {
    expect(app(CodexHarness::class)->identifier())->toBe(AgentHarnessIdentifier::Codex);
});

test('reports codex execution capabilities', function () {
    $capabilities = app(CodexHarness::class)->capabilities();

    expect($capabilities->models)->toBe([])
        ->and($capabilities->reasoningSettings)->toBe([])
        ->and($capabilities->executionOptions)->toBe(['ephemeral', 'streaming', 'heartbeat']);
});
...
test('executes with an explicitly isolated environment', function () {
    ...
    try {
        $agent = Agent::factory()->create(['harness' => AgentHarnessIdentifier::Codex]);

        app(CodexHarness::class)->execute($agent->project, $agent, 'Implement the task.');
...
test('refuses to execute for a persisted project path outside the workspace', function () {
    ...
    $agent = Agent::factory()->create(['project_id' => $project->id, 'harness' => AgentHarnessIdentifier::Codex]);
    Process::fake();

    expect(fn () => app(CodexHarness::class)->execute($project, $agent, 'Implement the task.'))->toThrow(UnsafeProjectPath::class);
...
test('streams output and renews heartbeats while executing', function () {
    ...
    Process::fake(['*' => Process::describe()->output('streamed output')->exitCode(0)->iterations(2)]);
    $agent = Agent::factory()->create(['harness' => AgentHarnessIdentifier::Codex]);
    ...
    expect($streamed)->not->toBeEmpty()
        ->and($heartbeats)->toBeGreaterThan(0);
});

test('normalizes the codex execution result', function () {
    Process::fake(['*' => Process::result(output: 'stdout-body', errorOutput: '', exitCode: 0)]);
    $agent = Agent::factory()->create(['harness' => AgentHarnessIdentifier::Codex]);

    $result = app(CodexHarness::class)->execute($agent->project, $agent, 'Implement the task.');

    expect($result->exitCode)->toBe(0)
        ->and(rtrim($result->output, "\n"))->toBe('stdout-body')
        ->and($result->errorOutput)->toBe('')
        ->and($result->externalRunId)->toBeNull()
...
test('resolves the codex harness from the container-bound resolver', function () {
    ...
    expect($harness)->toBeInstanceOf(CodexHarness::class);
});

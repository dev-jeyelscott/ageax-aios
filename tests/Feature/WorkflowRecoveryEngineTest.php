...
/**
 * Create a managed-project fixture for workflow recovery.
 */
function recoveryProject(string $name = 'Example'): Project
{
    return Project::create([
        'name' => $name,
        'path' => '/tmp/recovery-'.fake()->uuid(),
...
/**
 * Bind a focused workflow recovery test double.
 *
...
/**
 * Create a real temporary AIOS recovery Git repository.
 */
function recoveryRepository(): string
{
    $path = sys_get_temp_dir().'/aios-workflow-recovery-'.fake()->uuid();
...
/**
 * Create one workflow Task fixture.
 */
...
test('the workflow recovery scan is scheduled every five minutes', function () {
    ...
    expect($schedule->events()[0]->expression)
        ->toBe('*/5 * * * *');

    $registration = file_get_contents(base_path('bootstrap/app.php'));

    expect($registration)
        ->toContain("command('aios:recover-workflows')->everyFiveMinutes()");
});
...
test('a task stuck blocked past the anti-flake floor is detected as an open incident', function () {
    ...
    expect($incident)
        ...
        ->and($incident->failure_type)
        ->toBe('task_blocked')
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.incident_detected')
                ->exists(),
        )
        ->toBeTrue();
});
...
test('a task that only just became blocked is not yet flagged', function () {
    ...
    expect(RecoveryIncident::query()->count())
        ->toBe(0);
});
...
test('an expired worker lease is auto-recovered by the scan into a single resolved incident', function () {
    ...
    expect($task->refresh()->status)
        ...
        ->and($incident->root_cause_category)
        ->toBe('stale_lease')
...
test('a deterministically classified unsafe repository state escalates without invoking the recovery engineer', function () {
    ...
    recoveryMock(RecoveryEngineerRunner::class)
    ...
    expect($processed->status)
        ...
        ->toBe('unsafe_git_state')
        ->and($processed->recoverable)
        ->toBeFalse()
...
test('a disabled global Recovery Engineer agent blocks diagnosis cleanly instead of attempting a run', function () {
    ...
    expect($processed->status)
        ->toBe(RecoveryIncidentStatus::Escalated)
        ->and($processed->root_cause_category)
        ->toBe('configuration_environment')
        ->and($processed->recoverable)
        ->toBeFalse()
        ->and($task->refresh()->status)
        ->toBe(TaskStatus::Blocked)
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.blocked_agent_misconfigured')
                ->exists(),
        )
        ->toBeTrue();
});
...
test('a diagnosed managed-project defect is requeued without editing the AIOS repository', function () {
    $repositoryPath = recoveryRepository();
    $baseSha = trim(
    ...
    expect($processed->status)
        ...
        ->and($processed->resulting_task_transition)
        ->toBe('queued')
        ->and($task->refresh()->status)
        ...
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.recovered')
                ->exists(),
        )
        ->toBeTrue();
});
...
test('a validated AIOS code fix is committed before the task is requeued', function () {
    ...
    recoveryMock(RecoveryEngineerRunner::class)
        ...
        ->andReturnUsing(function (
            ...
            File::ensureDirectoryExists($worktreePath.'/app/Services');

            File::put(
                $worktreePath.'/app/Services/Example.php',
                "<?php\n\nreturn true;\n",
            );
    ...
    expect($processed->status)
        ...
        ->and($processed->commit_sha)
        ->not->toBe($baseSha)
        ->and($processed->fix_summary)
        ->toBe('Fixed the spurious blocking condition.')
        ->and($task->refresh()->status)
        ->toBe(TaskStatus::Queued)
        ->and(File::exists($repositoryPath.'/app/Services/Example.php'))
        ->toBeTrue()
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.fix_committed')
                ->exists(),
        )
        ->toBeTrue();
});

test('a fix that fails isolated AIOS validation is escalated and never reaches the live repository', function () {
    $repositoryPath = recoveryRepository();
    $project = recoveryProject();
    ...
    expect($processed->status)
        ...
        ->and($processed->commit_sha)
        ->toBeNull()
        ->and(File::exists($repositoryPath.'/private.pem'))
        ->toBeFalse()
        ->and($task->refresh()->status)
        ->toBe(TaskStatus::Blocked);
});
...
test('repeated recovery engineer execution failures are retried up to the bounded limit then escalated', function () {
    ...
    recoveryMock(RecoveryEngineerRunner::class)
        ...
        ->andReturn([
            'execution' => [
                'exit_code' => 1,
                'output' => '',
                'error_output' => 'Claude Code process exited with code 1.',
            ],
            'decision' => null,
        ]);
    ...
    expect($firstAttempt->status)
        ->toBe(RecoveryIncidentStatus::Detected)
        ->and($firstAttempt->attempt_count)
        ->toBe(1);

    $secondAttempt = app(WorkflowRecoveryEngine::class)->process($firstAttempt);

    expect($secondAttempt->status)
        ->toBe(RecoveryIncidentStatus::Escalated)
        ->and($secondAttempt->attempt_count)
        ->toBe(2)
        ->and($task->refresh()->status)
        ->toBe(TaskStatus::Blocked);
});
...
test('a diagnosis that throws still counts as a bounded attempt instead of retrying forever', function () {
    ...
    expect($firstAttempt->status)
        ...
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.diagnosis_exception')
                ->exists(),
        )
        ->toBeTrue();
...
test('recovering a later task cannot bypass an incomplete earlier task', function () {
    ...
    recoveryMock(RecoveryEngineerRunner::class)
        ...
        ->andReturn([
            ...
            'decision' => [
                'root_cause_category' => 'managed_project_defect',
                'root_cause_summary' => 'Not an AIOS defect.',
                'recoverable' => true,
                'fix_applied' => false,
                'changed_files' => [],
                'fix_summary' => null,
    ...
    expect($secondTask->refresh()->status)
        ->toBe(TaskStatus::Queued)
        ->and(app(ClaimTask::class)->handle($project, AgentRole::Coder))
        ->toBeNull();
});
...
test('a diagnosing incident whose recovery engineer run died mid-execution is reclaimed for a fresh diagnosis', function () {
    ...
    $run = AgentRun::create([
        ...
        'status' => AgentRunStatus::Running,
        'prompt_hash' => hash('sha256', 'stale'),
        'started_at' => now()->subMinutes(20),
    ]);

    app(WorkflowRecoveryEngine::class)->reclaimStaleClaims($project);

    expect($incident->refresh()->status)
        ->toBe(RecoveryIncidentStatus::Detected)
        ->and($incident->claimed_at)
        ->toBeNull()
        ->and($run->refresh()->status)
        ->toBe(AgentRunStatus::Interrupted)
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.claim_reclaimed')
                ->exists(),
        )
        ->toBeTrue();
});
...
test('a recently claimed diagnosing incident is left untouched by the reclaim pass', function () {
    ...
    expect($incident->refresh()->status)
        ->toBe(RecoveryIncidentStatus::Diagnosing);
});
...
test('a dirty tree attributable to another task auto-resumes that task instead of escalating', function () {
    ...
    $originTask->auditEvents()->create([
        'event_type' => 'task.coder_retry_exhausted',
        'payload' => [],
        'occurred_at' => now(),
    ]);
    ...
    $blockedTask->auditEvents()->create([
        'event_type' => 'task.blocked_dirty_repository',
        'payload' => [],
        'occurred_at' => now(),
    ]);
    ...
    expect($processed->status)
        ->toBe(RecoveryIncidentStatus::Recovered)
        ->and($processed->root_cause_category)
        ->toBe('stale_agent_attempt')
        ->and($originTask->refresh()->status)
        ->toBe(TaskStatus::Queued)
        ->and($blockedTask->refresh()->status)
        ->toBe(TaskStatus::Blocked)
        ->and(
            $project->auditEvents()
                ->where('event_type', 'task.stale_attempt_reclaimed')
                ->exists(),
        )
        ->toBeTrue();
});
...
test('recovery preserves the historical AgentRun and audit evidence from the original failed attempt', function () {
    ...
    $originalAttempt = TaskAttempt::create([
        'task_id' => $task->id,
        'number' => 1,
        'status' => 'failed',
    ...
    $originalRun = AgentRun::create([
        'project_id' => $project->id,
        'task_id' => $task->id,
        'role' => AgentRole::Coder,
        'status' => AgentRunStatus::Failed,
        'prompt_hash' => hash('sha256', 'original'),
        'started_at' => now()->subMinutes(10),
        'finished_at' => now()->subMinutes(9),
    ]);
    ...
    expect($originalAttempt->refresh()->status)
        ->toBe('failed')
        ->and($originalRun->refresh()->status)

...
use App\Services\RuntimeRecoverabilityPolicy;
...
/**
 * Create the minimum managed-project fixture required for runtime recovery policy.
 */
...
/**
 * Bind a focused runtime-recovery test double before WorkflowRecoveryEngine is resolved.
 *
...
/**
 * Persist one project-scoped application runtime incident through the real recorder.
 */
function runtimeRecoveryApplicationIncident(
    Project $project,
    string $source = 'route:projects.runtime',
): RecoveryIncident {
...
/**
 * Persist the exact allowlisted stale-worker runtime incident shape.
 */
function runtimeRecoveryExpiredWorkerIncident(Project $project): RecoveryIncident
{
    ...
    return app(RuntimeRecoveryIncidentRecorder::class)->record(
        ...
        agentWorker: $worker,
        evidence: [
            'message' => 'The worker lease expired before execution state was reconciled.',
        ],
    );
}

test('a bounded project application exception is classified as candidate ai repair', function () {
    $project = runtimeRecoveryPolicyProject();
    $incident = runtimeRecoveryApplicationIncident($project);

    $classification = app(RuntimeRecoverabilityPolicy::class)->classify($incident);
    ...
    expect($classification['category'])
        ...
        ->toBeTrue()
        ->and($classification['deterministic_repair'])
        ->toBeNull();
});
...
test('an allowlisted expired worker lease uses the existing deterministic stale-worker recovery path', function () {
    ...
    runtimeRecoveryPolicyMock(RecoveryEngineerRunner::class)
    ...
    runtimeRecoveryPolicyMock(StaleWorkerRecovery::class)
        ...
        ->once()
        ->withArgs(
            fn (Project $candidate, int $staleAfterSeconds): bool => $candidate->is($project)
                && $staleAfterSeconds > 0,
        )
        ->andReturn(1);
    ...
    expect($processed->status)
        ...
        ->toBe(RuntimeRecoverabilityClassification::KnownDeterministicRepair->value)
        ->and($processed->attempt_count)
        ->toBe(1)
        ->and($processed->fix_summary)
        ->toContain('Deterministic stale-worker recovery')
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.runtime_deterministic_repair_completed')
                ->exists(),
        )
        ->toBeTrue();
});

test('security database and authentication runtime evidence defaults to operator-only escalation', function (
    string $source,
    ...
    expect($processed->status)
        ...
        ->and($processed->attempt_count)
        ->toBe(0)
        ->and($processed->escalation_reason)
        ->not->toBeNull()
        ->and(
            AgentRun::query()
    ...
    'security credential boundary' => [
        ...
        'App\\Security\\CredentialException',
...
test('a runtime incident without stable fingerprint identity closes as non-actionable without recovery execution', function () {
    ...
    $incident = RecoveryIncident::create([
        ...
        'detected_at' => now(),
        'evidence' => [
            'message' => 'Legacy runtime evidence without a stable fingerprint.',
        ],
    ]);
    ...
    expect($processed->status)
        ->toBe(RecoveryIncidentStatus::Recovered)
        ->and($processed->root_cause_category)
        ->toBe(RuntimeRecoverabilityClassification::NonActionable->value)
        ->and($processed->recoverable)
        ->toBeFalse()
        ->and($processed->attempt_count)
        ->toBe(0)
        ...
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.runtime_non_actionable')
                ->exists(),
        )
        ->toBeTrue();
});
...
test('repeated deterministic runtime repair failures with unchanged evidence open the circuit breaker before the absolute retry ceiling', function () {
    ...
    expect($first->status)
        ...
        ->and($failures)
        ->toHaveCount(2)
        ->and($failures[0]->payload['no_progress']['consecutive_repeat_count'])
        ->toBe(0)
        ->and($failures[1]->payload['no_progress']['consecutive_repeat_count'])
        ->toBe(1)
        ->and($failures[1]->payload['no_progress']['failure_fingerprint'])
        ->toBe($failures[0]->payload['no_progress']['failure_fingerprint'])
        ->and(
            $project->auditEvents()
...
test('materially changed durable recovery evidence resets the runtime no-progress repeat window', function () {
    ...
    expect($second->status)
        ...
        ->toBe(0)
        ->and($failures[1]->payload['no_progress']['failure_fingerprint'])
        ->not->toBe($failures[0]->payload['no_progress']['failure_fingerprint'])
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.runtime_circuit_breaker_opened')
                ->exists(),
...
test('the existing recovery maximum attempt ceiling remains authoritative for runtime repair', function () {
    ...
    expect($processed->status)
        ...
        ->toBe(1)
        ->and($processed->escalation_reason)
        ->toBe('Bounded recovery attempt limit reached.')
        ->and(
            $project->auditEvents()
                ->where('event_type', 'recovery.runtime_circuit_breaker_opened')
                ->exists(),
...
test('the scheduled recovery pass escalates unscoped runtime incidents instead of leaving them invisible', function () {
    $incident = app(RuntimeRecoveryIncidentRecorder::class)->record(
        ...
        evidence: [
            'message' => 'The scheduled runtime recovery command failed.',
        ],
    );

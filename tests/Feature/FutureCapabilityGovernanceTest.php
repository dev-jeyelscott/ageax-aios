...
beforeEach(function (): void {
    config()->set(
        ...
        storage_path(
            'framework/testing/obsidian-future-governance-'
            .fake()->uuid(),
        ),
    );
});
...
function futureGovernanceTask(Project $project): Task
{
    return Task::create([
        ...
        'acceptance_criteria' => [
            'AIOS remains the durable workflow authority.',
        ],
 ...
 */
function expectFutureGovernanceClauses(
    string $path,
    array $clauses,
): void {
    $contents =
...
test('phase four governance contracts keep future capabilities subordinate to AIOS authority', function (): void {
    ...
    expectFutureGovernanceClauses(
        '.ai/rules/knowledge-improvements.md',
        [
...
test('persisted global system Agent authority contains only explicitly activated system roles', function (): void {
    $roles = Agent::query()
        ->whereNull('project_id')
        ->get()
        ->map(
            fn (Agent $agent): string => $agent->role->value,
        )
        ->sort()
        ->values()
        ->all();
...
test('approved global Agent role identities remain immutable', function (): void {
    foreach ([
        AgentRole::RecoveryEngineer,
        AgentRole::Orchestrator,
        AgentRole::KnowledgeArchitect,
        ...
        $agent->role = $replacement;

        expect(
            fn () => $agent->save(),
        )->toThrow(LogicException::class);

        expect($agent->fresh()?->role)
            ->toBe($role);
    }
});

test('Orchestrator cannot be persisted as a project Agent', function (): void {
    $project = futureGovernanceProject(
        'Orchestrator Project Scope',
    );

...
test('the agents index exposes all activated singleton global Agents by role', function () {
    $recoveryAgent =
        p5001ControllerAgent(
    ...
    $this
        ...
        ->assertInertia(
            fn (Assert $page) => $page
                ...
                ->where(
                    ...
                    function ($agents) use (
                        ...
                        return
                            ...
                            && $byRole->count() === 3
                            && (
                                $orchestratorPayload['id']
                                ?? null
                            ) ===
                                $orchestrator->id
                            && (
                                $orchestratorPayload['name']
                                ?? null
                            ) ===
                                'AIOS Global Orchestrator'
                            && (
                                $orchestratorPayload['runtime_status']
                                ?? null
                            ) === 'idle'
                            && (
                                $orchestratorPayload['open_incident_count']
                                ?? null
                            ) === 0
                            && (
                                $knowledgeArchitectPayload['id']
                                ?? null
                            ) ===
                                $knowledgeArchitect->id
                            && (
                                $knowledgeArchitectPayload['name']
                                ?? null
                            ) ===
                                'AIOS Knowledge Architect'
                            && (
                                $knowledgeArchitectPayload['runtime_status']
                                ?? null
                            ) === 'idle'
                            && (
                                $knowledgeArchitectPayload['open_incident_count']
                                ?? null
                            ) === 0
                            && (
                                $recoveryPayload['id']
                                ...
                                $recoveryAgent->id
                            && (
                                $recoveryPayload['name']
                                ?? null
                            ) ===
                                'AIOS Workflow Recovery Engineer'
                            && (
                                $recoveryPayload['open_incident_count']
                                ?? null
                            ) === 1
                            && (
                                $recoveryPayload['runtime_status']
                                ?? null
                            ) === 'working'
                            && data_get(
                                $recoveryPayload,
                            ...
                            && data_get(
                                $recoveryPayload,
                                'recent_activity.0.duration_seconds',
                            ) === 4;
                    },
                )
                ->where(
                ...
                ->where(
                    'active_incidents.0.id',
                    $incident->id,
                )
                ->where(
                    'active_incidents.0.status',
                    'diagnosing',
                )
                ->where(
                    'recent_events.0.id',
...
test('updating the Orchestrator bumps its configuration version and preserves global identity', function () {
    ...
    expect($agent->default_context)
        ->toBe(
            'Advisory Orchestrator context.',
        )
        ->and(
...
test('invoking the Recovery Engineer dispatches the existing manual recovery job', function () {
    ...
    Queue::assertPushed(
        ...
        fn (
            RunWorkflowRecoveryEngineerNow $job,
        ): bool => $job->agentId === $agent->id,
    );

...
{
    public function run(AssignSkillToAgent $assignSkill, AuditLogger $audit): void
    {
        foreach (Project::query()->lazyById() as $project) {
            DB::transaction(function () use ($project, $assignSkill, $audit): void {
                ...
                foreach ($this->definitions() as $roleValue => $definitions) {
                    foreach ($definitions as $definition) {
                        ...
                        $assignSkill->handle($agent, $skill);
                    }
                }
            }, attempts: 3);
        }

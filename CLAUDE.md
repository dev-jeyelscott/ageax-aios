...
- Every roadmap analysis, Project Manager `ticket_triage` attempt, implementation attempt, fix/retry attempt, and review must use a fresh Claude Code execution context when Claude Code is the selected harness, except an AIOS-owned Feature Goal session resumed for the same GoalRun and role. Do not rely on persistent Claude conversations for durable project state; provider sessions remain disposable runtime references.

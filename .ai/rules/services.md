...
Project Manager and Reviewer execution remain serial. Coder execution is serial by default and supports an authorized, validated per-project concurrency of 1 or 2 persisted on `Project::coder_concurrency`. Concurrent Coder admission above 1 slot is allowed only after `ParallelTaskSafetyEvaluator` proves the candidate Task is independent of every other currently active Coder Task on that project, and must preserve dependency/phase eligibility, authorization, worker leases, Context Budget enforcement, validation, and recovery. Dependency-related, overlapping, unsafe, or unknown impact always falls back to serial execution with a durable safety decision and reason. Each concurrent implementation must receive its own AIOS-owned isolated Git worktree or equivalent isolated workspace. Multiple Coders must never edit the same mutable checkout. Repository integration remains serialized per repository and AIOS-owned; an Agent or harness must never merge or integrate concurrent branches/worktrees itself. Lowering concurrency back to 1 stops new admission onto the second slot without interrupting or invalidating work already active there.

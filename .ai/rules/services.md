...
Every new execution attempt requires a fresh harness context and an immutable snapshot of the effective Agent configuration, selected harness/model/reasoning settings, bounded execution settings, default context, deterministic effective Skill identities/versions/order/content, and context schema version where applicable. A Feature Goal may reuse an isolated same-role provider session for the same GoalRun, but session identifiers are disposable runtime references and historical execution truth remains persisted run evidence and snapshots, never mutable Agent, Skill, or hidden conversation state.

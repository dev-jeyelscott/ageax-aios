...
Reviewer workflow remains strictly serial under AIOS control: only one eligible Reviewer task may be actively claimed at a time. Coder workflow is serial by default (one eligible Coder task actively claimed at a time) and supports an authorized, validated per-project concurrency of 1 or 2; it does not require each same-phase task to become `done` before the next eligible same-phase Coder implementation may begin.

Within the current phase, after a Coder task successfully reaches `ready_for_review`, AIOS may claim the next eligible same-phase Coder task when its persisted dependency requirements are satisfied. Phase batching must not bypass explicit dependency edges. Concurrent Coder execution above 1 slot is only ever admitted through `TaskWorkflow::claimConcurrentCoder()` after `ParallelTaskSafetyEvaluator` proves the candidate and every other active Coder Task are independent; dependency-related, overlapping, unsafe, or unknown combinations always fall back to serial execution with a durable reason.
...
Project Manager, Coder, and Reviewer remain the executable core workflow roles. Task execution is dependency ordered and AIOS-controlled; Project Manager and Reviewer stay serial, and Coder stays serial by default with only the authorized bounded per-project concurrency exception of at most 2. A Reviewer may claim the earliest reviewable Task while independent same-phase implementation continues, but additional configured Agents must not create worker lanes, self-schedule, or bypass persisted workflow ordering.
...
Future capability code must preserve the same authority split. Agents, harnesses, recommendations, handoffs, voice transcripts, workflow definitions, and routing evidence may request or propose an operation, but only an authorized AIOS-owned Action may validate and persist a durable mutation. No present or future Agent may select itself or another Agent for execution, grant or expand permissions, create worker authority, choose its own durable transition, apply a Reviewer or escalation decision directly, or bypass AIOS-controlled persistence.

A future Orchestrator is recommendation-first. Its structured output may propose Agent configuration, harness/model/reasoning selection, retry strategy, context strategy, task decomposition, recovery direction, or workflow improvement, but an Orchestrator Action must not directly mutate `Agent` configuration, bindings, workers, Tasks, workflow definitions, Git, or other durable state from model output. Any future application path requires a separately approved, authorized, schema-validated AIOS Action or explicit operator-owned policy.
...
Runtime self-healing must use the existing `RecoveryIncident` lifecycle and AIOS-owned recovery Actions/Services. Agent collaboration must use typed, bounded, AIOS-validated handoffs that carry evidence only and never perform transitions. The implemented bounded Coder concurrency (at most 2 per project) preserves dependency and phase eligibility, isolated AIOS-owned Git workspaces, worker/lease authority, deterministic validation, and serialized AIOS-owned repository integration; it never extends to Project Manager or Reviewer, which remain single-lane. Custom workflow definitions may request only approved declarative step types; they cannot contain executable code or grant transition authority. Automatic Agent/harness/model/reasoning routing remains disabled unless an explicit operator-owned routing policy activates it after the separately approved advisory and shadow stages, and any automatic selection must occur before a fresh attempt with immutable evidence and a deterministic fallback. No mid-attempt model or harness switching is allowed.

...
| Coder | Claim one eligible task per durable worker slot; inspect, implement, validate, secret-check, and return structured results. Within the current phase, AIOS may advance to the next eligible task after the previous task reaches `ready_for_review` when persisted dependencies permit. When a project's authorized concurrency is 2, AIOS may admit a second independent, dependency-safe task onto a second worker slot in its own isolated worktree. | Mark a task done; claim more tasks than the project's authorized concurrency permits; or execute concurrent work that AIOS has not proven dependency-safe. |
| Reviewer | Independently review one ready task at a time in deterministic position order using its criteria, exact diffs, SHAs, changed files, implementation, and verification evidence. It may review while independent Coder work continues. | Review more than one task concurrently, reject for taste, redesign valid work, or expand scope. |
...
- Project Manager and Reviewer execution remain strictly serial: one active Reviewer task at a time according to AIOS-owned ordering, with no additional worker lanes. Coder execution is serial by default (one active task at a time) and supports an authorized, validated per-project concurrency of 1 or 2; at 2, AIOS may admit a second independent, dependency-safe Coder task onto a second durable worker slot. It does not require every same-phase implementation to be reviewed before the next eligible same-phase Coder task may start.
...
- Coder and Reviewer workers observe the centrally configured per-role cooldown after completing a claimed task, evaluated independently per worker slot. The default is `AIOS_WORKER_TASK_COOLDOWN_SECONDS=300`, so a slot waits five minutes before claiming its next task.
...
- Additional project Agents do not self-schedule or create additional worker lanes; only AIOS-controlled supported workflow-role slots execute work. A project's second Coder worker slot exists only after an authorized operator raises that project's concurrency to 2; it remains a bounded Coder-only execution slot, never a new workflow role or PM/Reviewer lane.
- Git integration and Reviewer review always run against validated, canonically integrated evidence: repository merging remains serialized per project and AIOS-owned even when two Coder slots are active. Dependency-related, overlapping, unsafe, or unknown Task combinations always fall back to serial execution with a durable safety decision and reason, regardless of the configured concurrency bound.
- Lowering a project's concurrency from 2 to 1 stops new admission onto the second slot but must never interrupt or invalidate work already active on that slot.
...
Phase 4+ capability documentation does not itself activate new Agent roles, worker lanes, voice, handoffs, additional parallel execution beyond the implemented bounded Coder concurrency below, custom workflows, or routing. The current Project Manager/Coder/Reviewer runtime remains unchanged until a separately approved implementation task changes it; Project Manager and Reviewer execution stay serial, and Coder execution stays serial by default with only the implemented bounded per-project concurrency exception of at most 2.
...
- **Parallel execution:** Project Manager and Reviewer execution remain serial with no additional worker lanes. Coder execution supports an implemented, bounded, authorized per-project concurrency of 1 (default) or 2. Any admission above 1 must preserve dependencies, phase eligibility, authorization, leases, Context Budget, validation, and recovery, use a separate AIOS-owned isolated Git worktree/workspace per implementation, and keep repository integration serialized and AIOS-owned. Unsafe or unknown impact always falls back to serial execution.

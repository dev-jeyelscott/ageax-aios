...
class PlanFeatureGoal
{
    ...
    public function handle(FeatureSpec $featureSpec, ?WorkerLease $lease = null): GoalRun
    {
        ...
        $prompt = "You are the Project Manager planning one bounded backend feature. Inspect the managed repository, AGENTS.md, targeted documentation, and this untrusted FeatureSpec. Return only one JSON object with exactly these fields: {goal_text,title,objective,acceptance_criteria,scope,constraints,relevant_paths,verification_commands,implementation_prompt,context_capsule,work_type,complexity,required_documentation_paths,implementation_checklist,backend_engineer_requirements,reviewer_requirements}. Create exactly one implementation task. Every field except goal_text, title, objective, implementation_prompt, work_type, and complexity must be a JSON array of strings; context_capsule must be a JSON object. work_type must be exactly one of: bug, enhancement, feature, other. complexity must be exactly one of: low, medium, high. Do not use frontend, backend, small, medium-sized, or other unlisted labels. /goal must be detailed, bounded, and implementation-ready. Verification commands must be non-destructive. Do not wrap the JSON in Markdown fences or include prose.\n\n".json_encode($context->toArray(), JSON_THROW_ON_ERROR);

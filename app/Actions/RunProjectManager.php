...
class RunProjectManager
{
    ...
    private function normalizePlan(array $plan): array
    {
        ...
        foreach ($plan['phases'] ?? [] as $phaseIndex => $phase) {
            if (! is_array($phase) || ! is_array($phase['tasks'] ?? null)) {
                continue;
            }

            foreach ($phase['tasks'] as $taskIndex => $task) {
                if (is_array($task) && ! array_key_exists('depends_on', $task)) {
                    $plan['phases'][$phaseIndex]['tasks'][$taskIndex]['depends_on'] = [];
                }
            }
        }
    ...
    private function validatePlan(array $plan, int $taskPositionOffset = 0): void
    {
        $validator = validator($plan, [
            ...
            'phases.*.tasks.*.slice_classification' => ['nullable', Rule::in(['vertical_feature', 'infrastructure', 'refactor', 'migration', 'hardening', 'performance', 'observability', 'investigation'])],
            'phases.*.tasks.*.observable_behavior' => ['nullable', 'string'],
            'phases.*.tasks.*.verification_boundary' => ['nullable', 'string'],
            'phases.*.tasks.*.acceptance_contract' => ['nullable', 'string'],

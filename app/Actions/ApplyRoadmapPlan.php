        ...
        $result = DB::transaction(function () use ($project, $phases, $roadmap, $attempt, $structuredOutput, $isFinalBatch): array {
            ...
            foreach ($phases as $phasePosition => $phaseData) {
                ...
                foreach ($phaseData['tasks'] as $taskData) {
                    ...
                    $task = Task::create(['project_id' => $project->id, 'phase_id' => $phase->id, 'key' => 'TASK-'.str_pad((string) $position, 3, '0', STR_PAD_LEFT), 'position' => $position, 'title' => $taskData['title'], 'objective' => $taskData['objective'], 'work_type' => $taskData['work_type'] ?? null, 'complexity' => $taskData['complexity'] ?? null, 'acceptance_criteria' => $taskData['acceptance_criteria'], 'scope' => $taskData['scope'] ?? [], 'constraints' => $taskData['constraints'] ?? [], 'relevant_paths' => $taskData['relevant_paths'] ?? [], 'verification_commands' => $taskData['verification_commands'] ?? [], 'implementation_prompt' => $taskData['implementation_prompt'], 'context_capsule' => ['phase' => $phaseData['title'], 'objective' => $taskData['objective'], 'slice_classification' => $taskData['slice_classification'] ?? null, 'observable_behavior' => $taskData['observable_behavior'] ?? null, 'verification_boundary' => $taskData['verification_boundary'] ?? null, 'acceptance_contract' => $taskData['acceptance_contract'] ?? null, 'acceptance_criteria' => $taskData['acceptance_criteria'], 'scope' => $taskData['scope'] ?? [], 'constraints' => $taskData['constraints'] ?? [], 'relevant_paths' => $taskData['relevant_paths'] ?? [], 'verification_commands' => $taskData['verification_commands'] ?? [], 'obsidian_notes' => $this->safeObsidianNotes($taskData['obsidian_notes'] ?? []), 'completion_evidence' => $taskData['completion_evidence'] ?? null], 'status' => $completed ? TaskStatus::Done : TaskStatus::Queued, 'completed_at' => $completed ? now() : null]);
